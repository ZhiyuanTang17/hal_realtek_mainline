...
static void get_wdt_config_params(uint32_t target_ms, uint16_t *out_div_factor, uint8_t *out_cnt_limit)
{
    ...
    for (uint8_t limit = 0; limit <= 11; limit++) {
         ...
         * (max_cnt * (1 + div_factor) * 1000) / 32768 = target_ms
         * 1 + div_factor = (target_ms * 32768) / (max_cnt * 1000)
         */
        uint64_t numerator = (uint64_t)target_ms * CLK_FREQ_HZ;
        uint64_t denominator = (uint64_t)max_cnt * 1000ULL;
        ...
        uint64_t actual_ms = (max_cnt * (1ULL + div_factor) * 1000ULL) / CLK_FREQ_HZ;
